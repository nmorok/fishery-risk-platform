...
def run_scenario_monte_carlo(scenario_name, fitted_distributions, 
    ...
    # Disaster frequency from historical + climate multiplier
    disasters_per_year = df.groupby('sst_year').size()
    
    results = []
    
    for sim in range(n_simulations):
        # Sample base disaster count, then apply frequency multiplier
        ...
        n_disasters = max(1, n_disasters)  # At least 1 disaster
...
# Panel C: Mean appropriations by scenario
ax = axes[1, 0]
...
# Figure 2: Disaster frequency comparison
...
print("KEY FINDINGS")
print("="*80)
print(f"\nWith moderate climate change (+20%):")
print(f"  • Disaster frequency: +20% ({disasters_per_year.mean():.1f} → "
      f"{disasters_per_year.mean()*1.2:.1f} per year)")
print(f"  • Mean appropriation: +{moderate_increase:.1f}%")
print(f"  • Risk of >$100M years: "
      f"{comparison_data[0]['P(>$100M)']:.1%} → {comparison_data[1]['P(>$100M)']:.1%}")

print(f"\nWith high climate change (+40%):")
print(f"  • Disaster frequency: +40% ({disasters_per_year.mean():.1f} → "
      f"{disasters_per_year.mean()*1.4:.1f} per year)")
print(f"  • Mean appropriation: +{high_increase:.1f}%")
print(f"  • Risk of >$100M years: "
      f"{comparison_data[0]['P(>$100M)']:.1%} → {comparison_data[2]['P(>$100M)']:.1%}")
